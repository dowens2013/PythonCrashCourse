...
def greet_user():
    ...
    correct_username = input(f"Is {user_details['username']} the correct username? ")
    if correct_username == 'yes':
        security_response = int(input("Input security pin: "))
        if security_response == user_details['security_pin']:
            print(user_details)
        else:
            print("Incorrect pin")
            exit()
